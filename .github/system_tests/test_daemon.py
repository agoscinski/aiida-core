...
def validate_workchains(expected_results):
    ...
    for pk, expected_value in expected_results.items():
        ...
        try:
            ...
            print(
                f'* UNABLE TO RETRIEVE VALUE for workchain pk={pk}: I expected {expected_value}, '
                f'I got {type(exception)}: {exception}'
            )
            valid = False
            this_valid = False
            actual_value = None
    ...
    for calc in cached_calcs:
        ...
        if isinstance(calc, CalcJobNode):
            ...
            if not files_original:
                print(
                    f'Original calculation <{original_calc.pk}> does not have any raw inputs files '
                    'after being cached from.'
                )
                valid = False
