...
def test_code_show(run_cli_command, aiida_localhost, tmp_path, bash_path, aiida_code, code_type, file_regression):
    ...
    # Determine the filepath_executable used for normalization (platform-dependent)
    if code_type == 'containerized':
        exec_path = str(bash_path.absolute())
    elif code_type == 'installed':
        exec_path = '/remote/abs/path'
    else:
        exec_path = None
