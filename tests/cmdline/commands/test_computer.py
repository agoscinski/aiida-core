...
def generate_setup_options(ordereddict):
    ...
    """
    options = [ordereddict.get('auth', 'core.local')]
...
def test_setup_requires_transport(run_cli_command):
    """Setup does not silently select a transport plugin."""
    result = run_cli_command(computer_setup, ['--non-interactive'], raises=True)
...
def test_cancelled_configuration_removes_computer(run_cli_command, aiida_localhost, monkeypatch, command):
    ...
    if command is computer_setup:
        monkeypatch.setattr(orm.Computer, 'configure', Mock(side_effect=click.Abort()))
...
def test_setup_ssh_defaults_host_to_hostname(run_cli_command):
    """Dynamic SSH options retain the computer hostname as the default login host."""
    options = generate_setup_options(
        generate_setup_options_dict({'label': 'ssh_default_host', 'hostname': 'login.example.org', 'auth': 'core.ssh'})
    )
    result = run_cli_command(computer_setup, options)

    computer = orm.Computer.collection.get(label='ssh_default_host')
    assert computer.get_configuration()['host'] == 'login.example.org', result.output


@pytest.mark.parametrize('name', ('localhost', 'slurm-ssh'))
def test_ci_computer_setup_config(run_cli_command, tmp_path, name):
    """A transport subcommand accepts the setup YAML files, including authentication parameters."""
    config_path = pathlib.Path(__file__).resolve().parents[3] / '.github' / 'config' / f'{name}.yaml'
    config = config_path.read_text().replace('PLACEHOLDER_WORK_DIR', str(tmp_path))
    config = config.replace(f'label: {name}', f'label: ci-{name}')
    config += '\nauth_params:\n  safe_interval: 0\n'
    config_file = tmp_path / f'{name}.yaml'
    config_file.write_text(config)
    transport = 'core.ssh' if name == 'slurm-ssh' else 'core.local'

    result = run_cli_command(computer_setup, [transport, '--non-interactive', '--config', str(config_file)])

    computer = orm.Computer.collection.get(label=f'ci-{name}')
    assert computer.is_configured, result.output
    assert computer.get_configuration()['safe_interval'] == 0
    if name == 'slurm-ssh':
        assert computer.get_configuration()['host'] == 'localhost'


def test_setup_config_transport_mismatch(run_cli_command, tmp_path):
    """The transport in a YAML file must match the selected subcommand."""
    config_file = tmp_path / 'computer.yaml'
    config_file.write_text('label: mismatch\nhostname: localhost\nauth: core.local\nscheduler: core.direct\n')

    result = run_cli_command(
        computer_setup, ['core.ssh', '--non-interactive', '--config', str(config_file)], raises=True
    )

    assert 'does not match' in result.output
    assert ('mismatch',) not in orm.Computer.collection.list_labels()
...
def test_noninteractive_wrong_transport_fail(run_cli_command):
    ...
    options = generate_setup_options(options_dict)
    result = run_cli_command(computer_setup, options, raises=True)
    assert '`unknown_transport` is not a setup command' in result.output
...
"""
        ...
        options = ['core.local', '--non-interactive', '--config', os.path.realpath(handle.name)]
        run_cli_command(computer_setup, options)
...
def test_direct_interactive(run_cli_command, non_interactive_editor):
    ...
    result = run_cli_command(computer_setup, ['core.local'], user_input=user_input)
    assert result.exception is None, f'There was an unexpected exception. Output: {result.output}'
