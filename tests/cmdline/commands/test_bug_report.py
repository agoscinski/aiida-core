...
def test_check_broker_zeromq(monkeypatch, tmp_path, is_running, status):
    """Test ``_check_broker`` reads the ZeroMQ broker service files instead of connecting."""
    broker = _make_zeromq_broker(tmp_path, is_running=is_running, status=status)
    manager = SimpleNamespace(get_broker=lambda: broker)
