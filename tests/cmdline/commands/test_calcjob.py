...
class TestVerdiCalculation:
    ...
    def test_calcjob_gotocomputer(self):
        ...
        with patch(
            'aiida._core.orm.nodes.process.calculation.calcjob.CalcJobNode.get_transport',
            new=lambda _: raise_(NotExistent),
        ):
            result = self.cli_runner.invoke(command.calcjob_gotocomputer, options)
        ...
        with patch(
            'aiida._core.orm.nodes.process.calculation.calcjob.CalcJobNode.get_remote_workdir', new=lambda _: None
        ):
            result = self.cli_runner.invoke(command.calcjob_gotocomputer, options)
            assert result.exit_code == 1
