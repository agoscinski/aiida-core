...
- Phase 2b: ``system_loglevel`` handler-level filter on daemon log handler
...
class TestSystemLoglevelDaemonHandler:
    """Tests for ``system_loglevel`` as handler-level filter on the daemon log handler."""

    def test_daemon_handler_uses_system_loglevel(self, tmp_path):
        """When ``daemon=True``, the daemon handler should use ``system_loglevel`` (default: REPORT)."""
        ...
        # The daemon handler level should be REPORT (the default for system_loglevel),
        # NOT the old hardcoded 'DEBUG'
        assert captured_config['handlers']['daemon_log_file']['level'] == 'REPORT'
