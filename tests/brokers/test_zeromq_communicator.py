...
def running_zeromq_server(tmp_path_factory):
    tmp_path = tmp_path_factory.mktemp('zeromq-server')
...
class TestZeromqCommunicatorLifecycle:
    ...
    def test_init(self, running_zeromq_server):
        """Test communicator initialization."""
        communicator = ZeromqCommunicator(router_endpoint=running_zeromq_server.router_endpoint)
        communicator.start()
    ...
    def test_context_manager(self, running_zeromq_server):
        """Test communicator as context manager."""
        with ZeromqCommunicator(router_endpoint=running_zeromq_server.router_endpoint) as communicator:
            assert communicator.is_closed() is False

        assert communicator.is_closed() is True

    def test_close_idempotent(self, running_zeromq_server):
        """Test close is idempotent."""
        comm = ZeromqCommunicator(router_endpoint=running_zeromq_server.router_endpoint)
        comm.start()
        comm.close()
...
class TestZeromqCommunicatorMessaging:
    ...
    @pytest.fixture
    def zeromq_comm(self, running_zeromq_server):
        comm = ZeromqCommunicator(router_endpoint=running_zeromq_server.router_endpoint)
        comm.start()
...
class TestZeromqCommunicatorRoundTrip:
    ...
    @pytest.fixture
    def sender_and_worker(self, running_zeromq_server):
        sender = ZeromqCommunicator(router_endpoint=running_zeromq_server.router_endpoint, client_id='sender')
        sender.start()

        worker = ZeromqCommunicator(router_endpoint=running_zeromq_server.router_endpoint, client_id='worker')
        worker.start()
