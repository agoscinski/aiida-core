...
Tests cover:
- Phase 1a: New ``logging.terminal_loglevel`` and ``logging.system_loglevel`` options
...
class TestNewLoggingOptions:
    ...
    def test_system_loglevel_exists(self):
        """The ``logging.system_loglevel`` option should exist."""
        option = get_option('logging.system_loglevel')
        assert isinstance(option, Option)

    def test_system_loglevel_default(self):
        """The ``logging.system_loglevel`` option should default to ``REPORT``."""
        option = get_option('logging.system_loglevel')
        assert option.default == 'REPORT'

    def test_new_options_are_not_advanced(self):
        """The new routing options should NOT be marked as advanced."""
        for name in ('logging.terminal_loglevel', 'logging.system_loglevel'):
    ...
    def test_new_options_in_option_names(self):
        ...
        assert 'logging.system_loglevel' in names
