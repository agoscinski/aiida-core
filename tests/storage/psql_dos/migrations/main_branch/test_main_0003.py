...
def test_rename_ssh_async_transport(perform_migrations: PsqlDosMigrator, ssh_dir):
    ...
    uuids = {label: get_new_uuid() for label in ('async', 'async_two', 'legacy', 'local')}
    ...
    assert auth_params['host'] == f'aiida-migrated-{uuids["legacy"]}'
    ...
    # One entry, for the one legacy computer: the renamed ones need none.
    assert entry.count('\nHost ') == 1
