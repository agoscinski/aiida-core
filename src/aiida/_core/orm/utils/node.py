...
def get_type_string_from_class(class_module, class_name):
    ...
    prefixes = ('aiida._core.orm.nodes.', 'aiida.orm.nodes.', 'aiida.orm.core.')

    # Sequentially and **in order** strip the prefixes if present
    for prefix in prefixes:
        type_string = type_string.removeprefix(prefix)

    # This needs to be here as long as `Data` subclasses live in `aiida._core.orm.nodes.data` because all the
