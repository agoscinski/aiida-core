...
class SqlaQueryBuilder(BackendQueryBuilder):
    ...
    def _get_is_head_filter_expr(self, alias: AliasedClass, operator: str, value: t.Any) -> ColumnElement[bool]:
        ...
        if operator != '==':
            msg = f'Unknown operator {operator} for filters on is_head'
            raise ValueError(msg)
        if not isinstance(value, bool):
            msg = f'Value for is_head filter has to be a boolean (you gave {value})'
            raise TypeError(msg)
