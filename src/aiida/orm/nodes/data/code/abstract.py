...
__all__ = ('AbstractCode', 'Code')


class Code(Data, metaclass=abc.ABCMeta):
    """Abstract interface representing an executable code.

    This is the user-facing base class for all code plugins. Use it for ``isinstance`` checks and type hints instead
    of :class:`AbstractCode`, which additionally carries the shared implementation. The class itself is never stored
...
class AbstractCode(Code):
    """Abstract base implementation for code plugins."""
