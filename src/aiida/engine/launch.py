...
def _get_queue_info_for_submit(process_inited: Process) -> tuple[str, str] | None:
    ...
    if 'default' not in queue_config:
        raise InvalidOperation(
            'Cannot submit: no default queue configured for this profile. ' 'Create one with: verdi broker queue create'
        )

    user_queue = 'default'  # For now, always use the default queue
