...
class ZeromqCommunicator(kiwipy.Communicator):  # type: ignore[misc]
    ...
    def __init__(
        ...
        # Correlation map: message ID -> Future.  When task_send() or
        # rpc_send() fire a message, a Future is stored here keyed by the
    ...
    def close(self) -> None:
        ...
        if self._loop is not None and self._loop.is_running():
            if threading.current_thread() is self._loop_thread:
                ...
                gate.result(timeout=LOOP_TIMEOUT)
    ...
    def _do_close_on_loop(self) -> None:
        ...
        # Cancel pending futures and their timeouts
    ...
    # ------------------------------------------------------------------
    # Pending-future timeout management (loop thread only)
    # ------------------------------------------------------------------

    def _schedule_timeout(self, msg_id: str) -> None:
        """Schedule a timeout for a pending future. Must be called on the loop thread."""
        if self._task_timeout is None or self._loop is None:
            return
    ...
    def _cancel_timeout(self, msg_id: str) -> None:
        """Cancel a pending timeout. Must be called on the loop thread."""
        handle = self._timeout_handles.pop(msg_id, None)
    ...
    def task_send(self, task: Any, no_reply: bool = False) -> Future[Any] | None:
        ...
        def _do() -> Future[Any] | None:
            ...
            if not no_reply:
                ...
                self._schedule_timeout(task_id)
            self._send(msg)
