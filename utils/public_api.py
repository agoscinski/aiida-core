...
For AiiDA 2, anything importable directly from ``aiida`` or a second-level
package such as ``aiida.orm`` is public API. For AiiDA 3, the same rule applies
except within ``aiida.common``: every module whose dotted path has no
underscore-prefixed segment is public, and resources are recorded under their
full module path instead of being flattened onto ``aiida.common``.

A module's ``__all__`` is authoritative; otherwise, public names are inferred
from its top-level definitions and imports. The script does not import AiiDA.
For public classes, it also recurses into public members and nested classes.
"""
...
class SourceResolver:
            ...
            if isinstance(statement, ast.ImportFrom):
                ...
                for alias in statement.names:
                    if alias.name == '*':
                        ...
                        imported_symbol = self.resolve_symbol(imported_module, symbol_name, seen=seen)

                        if imported_symbol is not None:
                            self._symbol_cache[cache_key] = imported_symbol
                    ...
                    imported_symbol = self.resolve_symbol(imported_module, alias.name, seen=seen)
    ...
    def extract_class_resources(self, class_node: ast.ClassDef, path: str) -> list[ApiResource]:
        ...
        resources: dict[str, ApiResource] = {
            path: ApiResource(path=path, kind='class', signature=_get_constructor_signature(class_node))
        }
...
def _get_default_output(aiida_major_version: int) -> Path:
    """Return the default output path for an AiiDA major version."""
    return Path(__file__).resolve().parents[1] / f'public_api_v{aiida_major_version}.json'
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        usage='python utils/public_api.py {diff|extract} --aiida-major-version {2|3} ...',
        description=__doc__,
    )
    ...
    diff_parser.add_argument(
        ...
        choices=(2, 3),
        required=True,
        help='Public API convention to use when extracting the current checkout.',
    )
    diff_parser.add_argument(
        '--src-root',
        type=Path,
    ...
    diff_parser.add_argument(
        '--module',
        help='Only compare resources in this module, for example `aiida.common`.',
    )
    diff_parser.add_argument(
        '--exit-code',
        action='store_true',
    ...
    extract_parser.add_argument(
        ...
        choices=(2, 3),
        required=True,
        help='Public API convention to use for extraction.',
    )
    extract_parser.add_argument(
        '--src-root',
        type=Path,
    ...
    extract_parser.add_argument(
        '--output',
        type=Path,
        help='Path of the JSON file to write (default: public_api_v<major>.json).',
    )
    extract_parser.add_argument(
        '--module',
        help='Only include resources in this module, for example `aiida.common`.',
    )
...
def _strip_self(arguments: ast.arguments) -> ast.arguments:
    """Return a copy of ``arguments`` with the leading ``self`` parameter removed."""
...
def _get_constructor_signature(class_node: ast.ClassDef) -> str | None:
    """Return the ``__init__`` signature of a class, excluding ``self``, if one is defined."""
    for statement in class_node.body:
        if isinstance(statement, (ast.FunctionDef, ast.AsyncFunctionDef)) and statement.name == '__init__':
            signature = ast.unparse(_strip_self(statement.args))
...
class ApiExtractor:
    ...
    def __init__(self, src_root: Path, aiida_major_version: int) -> None:
        """Initialize the extractor."""
        self.src_root = src_root
        self.aiida_major_version = aiida_major_version
    ...
    @staticmethod
    def get_public_modules(package: Path) -> list[tuple[Path, str]]:
        """Return public modules below ``package`` as ``(path, suffix)`` pairs."""
        modules: list[tuple[Path, str]] = [(package / '__init__.py', '')]

        for path in sorted(package.rglob('*.py')):
            relative = path.relative_to(package)

            if path.name == '__init__.py':
                if relative.parent == Path('.') or any(part.startswith('_') for part in relative.parent.parts):
                    continue
                modules.append((path, '.'.join(relative.parent.parts)))
                continue

            module_parts = (*relative.parent.parts, path.stem)
            if any(part.startswith('_') for part in module_parts):
                continue
            modules.append((path, '.'.join(module_parts)))
    ...
    def build_payload(self) -> dict[str, Any]:
        ...
        for package in self.get_second_level_packages():
            ...
            if self.aiida_major_version == 2 or package.name != 'common':
                modules[module_name] = self.extract_module_api(package / '__init__.py', module_name)
                continue

            for path, suffix in self.get_public_modules(package):
                public_module_name = f'{module_name}.{suffix}' if suffix else module_name
                modules[public_module_name] = self.extract_module_api(path, public_module_name)
...
def _filter_payload(payload: dict[str, Any], module: str | None) -> dict[str, Any]:
    """Return ``payload`` restricted to resources below ``module``."""
    if module is None:
        return payload

    module = module.removesuffix('.')
    if module != 'aiida' and not module.startswith('aiida.'):
        message = f'Module should be `aiida` or start with `aiida.`, got: {module}'
        raise ValueError(message)

    prefix = f'{module}.'
    resources = SnapshotDiffer.build_resource_map(payload)
    return {
        'resources': {
            path: {'kind': resource.kind, 'signature': resource.signature}
            for path, resource in resources.items()
            if path == module or path.startswith(prefix)
        }
    }
...
class SnapshotDiffer:
    ...
    def is_changed_resource(old: ApiResource, new: ApiResource) -> bool:
        ...
        # Gate on ``old.kind`` rather than ``old.signature`` so a class that gains its own
        # ``__init__`` (``old.signature`` is ``None`` when the constructor was inherited) is still
        # flagged, while legacy ``unknown`` entries that never recorded a signature are ignored.
        # This is intentionally conservative: adding an optional-only ``__init__`` also trips here.
        if old.kind not in {'', 'unknown'} and old.signature != new.signature:
            return True
    ...
    def format_diff(diff: ApiDiff, baseline: Path) -> str:
        ...
        if diff.removed or diff.changed:
            lines.append('Changes possibly break the public API.')
        else:
            lines.append('Changes extend the public API.')
...
def _extract_snapshot(src_root: Path, output: Path, aiida_major_version: int, module: str | None) -> None:
    """Extract the public API snapshot to a file."""
    _validate_src_root(src_root)
    payload = _filter_payload(ApiExtractor(src_root, aiida_major_version).build_payload(), module)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(payload, indent=2, sort_keys=True) + '\n', encoding='utf8')


def _diff_snapshots(
    file1: Path, file2: Path | None, src_root: Path, aiida_major_version: int, module: str | None
) -> ApiDiff:
    ...
    baseline = _filter_payload(SnapshotDiffer.load_payload(file1), module)
    ...
    comparison = _filter_payload(SnapshotDiffer.load_payload(file2), module)
...
def main() -> None:
    ...
    if arguments.command == 'extract':
        output = arguments.output or _get_default_output(arguments.aiida_major_version)
        _extract_snapshot(arguments.src_root, output, arguments.aiida_major_version, arguments.module)
        return
