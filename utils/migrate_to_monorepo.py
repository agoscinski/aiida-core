...
jobs:
  ...
  pytest:
    ...
    - name: Install aiida-atomistic for backward-compat shims
      # Core suites import moved types via shims (see ci-code.yml);
      # no-op for other packages (condition false).
      if: ${{ inputs.package == 'aiida-core' }}
      run: uv run --project aiida-core pip install -e ./aiida-atomistic
      shell: bash
