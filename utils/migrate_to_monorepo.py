...
jobs:
  ...
  pytest:
    ...
    - name: Sync aiida-atomistic venv for backward-compat shims
      # Core suites import moved types via shims (see ci-code.yml).
      # No-op for other packages (condition false). Plain pip is banned:
      # it ignores `tool.uv.sources` and overwrites local core with PyPI.
      if: ${{ inputs.package == 'aiida-core' }}
      run: |
        uv sync --project aiida-atomistic --locked --extra tests
        echo "PATH=$GITHUB_WORKSPACE/aiida-atomistic/.venv/bin:$PATH" >> $GITHUB_ENV
      shell: bash
