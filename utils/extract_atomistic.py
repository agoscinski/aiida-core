...
Usage:
    ...
    python utils/extract_atomistic.py --execute --commit      # all phases, one commit per phase

Phases (also the suggested commit split for the automatable part):
...
# One commit per phase keeps `move` a pure-rename commit (reviewable with
# `git show --find-renames`) and isolates mechanical rewrites from new files.
COMMIT_MSGS = {
...
class Ctx:
    ...
    def commit_phase(self, phase: str, paths: list[str]) -> None:
        """Stage `paths` and commit the phase, skipping cleanly when empty."""
        for path in paths:
            ...
            self.git('add', path)
        if not self.execute:
            print(f'$ git commit -m "{COMMIT_MSGS[phase]}"')
            return
        ...
        if staged != 0:
            self.git('commit', '-m', COMMIT_MSGS[phase])
        else:
            print(f'skip (nothing staged): {phase}')
...
def main() -> int:
    ...
    parser.add_argument(
        '--commit',
        action='store_true',
        help='commit each executed phase separately (one commit per phase)',
    )
    ...
    if args.commit and args.execute and not working_tree_clean():
        ...
        print('Commit or stash them first so each phase lands as its own clean commit.')
        return 1
    ...
    for name in selected:
        ...
        phases[name](ctx)
        if args.commit:
            ctx.commit_phase(name, stage_paths[name])
